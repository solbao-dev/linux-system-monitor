...
**<해결 방법>**  
패키지 매니저 설치 대신 **UTM 공식 홈페이지에서 `.dmg` 파일을 다운로드**하여, 유저 전용 개인 폴더(`~/Applications/`)에 수동으로 복사 배치함으로써 관리자 권한 없이 설치를 성공시켰습니다.
...
### 📷 우분투 세부 환경 검증 스크린샷
![우분투 세부 환경 검증 화면](./screenshots/ubuntu-system-check.png)
---
...
**📌 핵심 검증 포인트 1 (ACL 활성화 기호 `+`)**  
`ls -ld` 명령어를 통해 각 보안 디렉토리(`upload_files`, `api_keys`, `/var/log/agent-app`)의 권한 식별자 맨 끝에 확장 접근 제어가 정상 가동 중임을 뜻하는 **플러스(`+`) 표식**을 검증 완료했습니다.

**📌 핵심 검증 포인트 2 (상세 그룹 분리)**  
공통 협업 영역은 `agent-common` 그룹, 핵심 보안 및 로그 영역은 `agent-core` 소유로 분리 지정되었습니다.

**📌 핵심 검증 포인트 3 (미래 권한 자동 상속)**  
`getfacl` 조회 결과, 하단에 `default:group:agent-common:rwx` 장부가 명시되어 있어 향후 유입되거나 생성될 하위 파일에도 수동 권한 부여 없이 보안 정책이 자동으로 누락 없이 상속됨을 증명합니다.
...
그룹: agent-core  
권한: 750 (-rwxr-x---)  
크론 실행자: agent-admin (agent-core 그룹 소속으로 실행 권한 확보)
...
* 프로세스 상태 확인 (agent-app 실행 상태 확인, 비정상 시 exit 1)
* TCP LISTEN 상태 확인 (15034 포트 수신 확인, 비정상 시 exit 1)
* 방화벽(UFW) 활성화 상태 점검 (비활성 시 경고 출력 후 계속 진행)
* 자원 수집 및 임계값 검증 (CPU > 20%, MEM > 10%, DISK > 80% 시 [WARNING] 출력)
* CPU 사용률 확인 
* MEM 사용률 확인
* DISK 사용률 확인
* 로그 정형화 기록 (`/var/log/agent-app/monitor.log`)
...
# 보너스 2 – clean.sh  
*인프라 생명주기 관리 및 방어적 프로그래밍*  


* **필수성:** 상용 서비스는 법적 규제 및 CCTV 보존 기한과 유사하게 '시간 기반 파일 수명 주기(Retention)' 관리가 필수적이며, 이를 통해 디스크 포화를 선제적으로 방어
* **기대 효과:** 리눅스 `find` 엔진과 조건문을 활용해 7일 경과 로그는 압축 격리하고 30일 경과 파일은 영구 삭제하는 파이프라인을 구축하며, 어떤 예외 상황(파일 부재 등)에서도 인프라 가동성이 중단되지 않는 '방어적 프로그래밍'의 본질을 이해함

| 항목 | 🎥 아파트 CCTV 시스템 | 💻 우분투 가상 서버 (`clean.sh`) |
| :--- | :--- | :--- |
| **수집 대상** | 매 순간 녹화되는 **CCTV 영상 파일** | 매 분마다 적재되는 **시스템 가동 로그 파일** |
| **법적/정책적 규제** | 개인정보보호법 준수 (보통 30일 보관) | 기업 보안 감사 지침 및 인프라 규제 준수 |
| **1단계 (7일 경과)** | 용량 최적화를 위해 화질을 낮춰 압축 보관 | 텍스트 용량 절감을 위해 `.gz` 압축 후 아카이브 폴더 격리 |
| **2단계 (30일 경과)** | 유통기한 만료에 따른 자동 영구 삭제 | 하드디스크 포화 방지를 위해 `rm -f` 명령어로 영구 파기 |
| **방어적 설계 (예외 처리)** | 녹화할 영상이 없어도 시스템이 다운되지 않음 | 처리 대상 로그가 0개여도 에러 없이 안전하게 프로세스 유지 |



<핵심기술>  
리눅스 커널 커맨드인 find 엔진의 -mtime 파라미터를 활용해 누적 일수를 정밀 추적  
7일 경과 로그는 스토리지 절약을 위해 gzip으로 결합 압축 후 별도 아카이브 경로(.../archive/)로 강제 격리 이송  
30일이 경과한 백업본은 선입선출(FIFO) 규칙에 따라 영구 삭제 처리

<방어적 프로그래밍 : 시스템의 연속성을 보장하기 위해 3대 예외 처리>  
실행 계정의 권한 부족 시 안전한 조기 종료(exit 1)  
대상 아카이브 폴더 부재 시 크래시 없이 자동 생성(mkdir -p)  
처리 대상 파일이 0개(Null)일 때 시스템 에러를 뿜지 않고 안내 메시지와 함께 안전하게 기동을 종료(missingok 메커니즘 자바스크립트/쉘 구현화)

작업명령어 확인 
```bash
cat /home/agent-admin/agent-app/bin/clean.sh
```
출력결과
```bash
====== LOG RETENTION POLICY EXECUTION ======
[INFO] 아카이브 디렉토리가 부재하여 자동 생성합니다: /var/log/monitor/agent-app/archive
[INFO] 7일 이상 경과된 압축 대상 로그 파일이 없습니다. (안전하게 패스)
[INFO] 30일 이상 경과된 삭제 대상 아카이브 파일이 없습니다. (안전하게 패스)
[OK] 로그 보존 정책 프로세스가 안전하게 종료되었습니다.
```
스크린샷
